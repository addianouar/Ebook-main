...
export const Purchase = () => {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      totalSeconds--;
      if (totalSeconds < 0) totalSeconds = 47 * 3600 + 59 * 60 + 59;
      setTimeLeft({
  ...
  const handlePayPalPayment = async () => {
    const { value: formValues } = await Swal.fire({
      ...
      preConfirm: () => {
        ...
        if (!email || !phone) Swal.showValidationMessage("Please fill out both fields!");
        if (!emailRegex.test(email)) Swal.showValidationMessage("Please enter a valid email!");
        return { email, phone };
      },
    });
    ...
    if (formValues) {
      try {
        ...
        Swal.fire({ icon: "success", title: "Redirecting to PayPal...", showConfirmButton: false, timer: 1500 });
        setTimeout(() => {
          window.open(
            "https://www.paypal.com/cgi-bin/webscr?cmd=_xclick&business=saraalaoui.paypal@gmail.com&amount=76&currency_code=EUR&item_name=Cupcake+Evolution+Vol+1",
            "_blank"
          );
        }, 1500);
      } catch {
        Swal.fire({ icon: "error", title: "Error", text: "Failed to send your info. Please try again.", confirmButtonColor: "#C5912C" });
      }
    }
  };
  ...
  const handleWhatsAppContact = () => {
    const message = encodeURIComponent(
      "Hello! I'm interested in the ebook 'Cupcake Evolution Volume 1' and would like to pay via bank transfer. Can you please provide the details?"
    );
    window.open(`https://wa.me/+212664576477?text=${message}`, "_blank");
  };

  return (
    <section id="purchase" className="relative py-16 lg:py-24 bg-[#F2EFE8] flex justify-center items-center">
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-12 relative z-10 flex flex-col items-center">
        ...
        <div className="bg-[#651C32] text-white px-6 py-3 rounded-full shadow-lg flex items-center gap-3 animate-pulse mb-8">
          ...
          <span className="font-semibold tracking-wide">{data.timerTitle}</span>
          <span className="text-[#C5912C] font-bold text-lg ml-2">
        ...
        <div className="flex flex-col lg:flex-row items-center justify-center gap-12 lg:gap-16 w-full">
          ...
          <div className="flex justify-center w-full lg:w-auto">
            <div
              ...
              style={{ transform: `rotateY(${rotate.y}deg) rotateX(${rotate.x}deg) perspective(800px)` }}
            >
              <div className="absolute inset-0 bg-[#C5912C]/20 rounded-3xl blur-3xl transform rotate-3 scale-110 animate-luxury-glow"></div>
              <img src={ebookCover} alt="Ebook Cover" className="relative z-10 w-full rounded-2xl shadow-xl object-cover" />
            </div>
          </div>

          {/* Purchase Card */}
          <Card className="bg-white/95 border border-[#C5912C]/30 p-6 sm:p-8 xl:p-10 shadow-2xl rounded-2xl hover:shadow-gold transition-all duration-500 w-full max-w-lg">
            {/* Title & Price */}
            ...
            <div className="flex flex-col sm:flex-row gap-4 mt-6">
              <LuxuryButton className="w-full sm:w-1/2 bg-[#C5912C] hover:bg-[#B47B24] text-white py-4 rounded-2xl font-semibold flex items-center justify-center gap-2 text-center whitespace-normal" onClick={handlePayPalPayment}>
                <CreditCard size={18} /> {data.paypal}
              </LuxuryButton>
              <LuxuryButton className="w-full sm:w-1/2 bg-[#25D366] hover:bg-[#1DA851] text-white py-4 rounded-2xl font-semibold flex flex-col sm:flex-row items-center justify-center gap-2 text-center whitespace-normal" onClick={handleWhatsAppContact}>
